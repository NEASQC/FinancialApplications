...
def hardware_efficient_ansatz_test(**kwargs):
    """
    ...
    """
    n_inputs = kwargs.get("n_inputs")
    n_qubits_by_inputs = kwargs.get("n_qubits_by_inputs")
    n_layers = kwargs.get("n_layers")
    n_qubits = n_qubits_by_inputs * n_inputs
    # Begin the QLM program
    pqc = qlm.Program()
    qbits = pqc.qalloc(n_qubits)
    features_names = ["feature_{}".format(input_) for input_ in range(n_inputs)]
    features = [pqc.new_var(float, feature) for feature in features_names]
    ...
    for layer_ in range(n_layers):
        input_index = []
        for input_ in range(n_inputs):
            qubit_index = []
            for qubit_ in range(n_qubits_by_inputs):
    ...
    for input_ in range(n_inputs):
        ...
        for qubit_ in range(n_qubits_by_inputs):
            pqc.apply(
                ...
                qbits[input_ * n_qubits_by_inputs + qubit_]
            )
    # Creating Layers
    for layer_ in range(n_layers):
        for input_ in range(n_inputs):
            # Variational Layer
            for qubit_ in range(n_qubits_by_inputs):
                # For each input reply alog the number of qubits for input
                pqc.apply(
